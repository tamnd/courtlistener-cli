// Package cli assembles the courtlistener command tree from the courtlistener
...
// NewApp assembles the kit application from the courtlistener domain. The
...
//
// To add a command, declare it in courtlistener/domain.go with kit.Handle and it
// appears here automatically. Reach for app.AddCommand only for a verb that does
// not fit the emit-records shape, the way version does below.
func NewApp() *kit.App {
	id := courtlistener.Domain{}.Info().Identity
	id.Version = Version

	app := kit.New(id)
	(courtlistener.Domain{}).Register(app)
