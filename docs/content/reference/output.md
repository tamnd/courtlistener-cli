...
describes what users get. Pick a format with `-o`, or let courtlistener choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
courtlistener <command> -o table   # aligned columns for reading
courtlistener <command> -o jsonl   # one JSON object per line, for piping
courtlistener <command> -o json    # a single JSON array
courtlistener <command> -o csv     # spreadsheet friendly
courtlistener <command> -o tsv     # tab-separated
courtlistener <command> -o url     # just the URL column
courtlistener <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
courtlistener <command> --fields id,title,url
```
...
```bash
courtlistener <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
courtlistener <command>            # a table, because this is a terminal
courtlistener <command> | wc -l    # JSONL, because this is a pipe
```
